...
(function () {
  ...
  function injectMarkup() {
    ...
    wrap.innerHTML = `
      ...
      <aside id="shared-drawer" class="shared-drawer fixed top-0 right-0 h-full w-full max-w-[520px] bg-slate-900 border-l border-slate-800 z-[60] overflow-y-auto">
      ...
      <div id="shared-evidence-backdrop" class="shared-mb fixed inset-0 bg-black/60 z-[65] flex items-center justify-center p-4">
        <div class="bg-slate-900 border border-slate-800 rounded-xl w-full max-w-2xl shadow-2xl max-h-[85vh] flex flex-col">
          <div class="px-5 py-4 border-b border-slate-800 flex items-center justify-between flex-shrink-0">
            <div class="flex items-center gap-2.5 min-w-0">
              ...
              <div class="min-w-0">
                <div class="text-[10px] uppercase tracking-wider text-slate-500" id="shared-evidence-kind">Evidence</div>
                <div class="text-sm font-semibold text-slate-100 truncate" id="shared-evidence-title">—</div>
              </div>
            </div>
            <button id="shared-evidence-close" class="p-1 rounded hover:bg-slate-800 text-slate-400 hover:text-slate-200 flex-shrink-0">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
          </div>
          <div id="shared-evidence-body" class="p-5 overflow-y-auto flex-1"></div>
          <div class="px-5 py-3 border-t border-slate-800 flex items-center justify-between flex-shrink-0">
            <span class="text-[11px] text-slate-500">Simulated for prototype · in production this opens the source system</span>
            <button id="shared-evidence-dismiss" class="px-3 py-1.5 rounded-lg bg-slate-800 text-slate-200 hover:bg-slate-700 text-sm">Close</button>
          </div>
        </div>
      </div>
  ...
  function openTaskDrawer(id) {
    ...
    const reviewBlock = isPending ? `
      <div class="bg-orange-500/10 border border-orange-500/40 rounded-lg p-4">
        <div class="flex items-center gap-2 mb-2">
          <svg class="w-4 h-4 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
          <span class="text-[11px] font-semibold uppercase tracking-wider text-orange-300">Review</span>
          <span class="text-[10px] text-orange-300/70">AI FLAGGED THIS — ACCEPT OR DROP</span>
        </div>
        <div class="flex items-center gap-2 mb-3 flex-wrap">
          ${hasDraft ? `
            <button class="td-action px-3 py-1.5 rounded-lg bg-green-500/20 text-green-300 border border-green-500/40 hover:bg-green-500/30 text-sm font-medium" data-action="approve-send" data-id="${t.id}">✓ Approve &amp; send</button>
            <button class="td-action px-3 py-1.5 rounded-lg bg-slate-800 text-slate-300 border border-slate-700 hover:bg-slate-700 text-sm inline-flex items-center gap-1.5" data-action="edit-in-composer" data-id="${t.id}">
              Edit + approve
          ...
          ` : `
            <button class="td-action px-3 py-1.5 rounded-lg bg-green-500/20 text-green-300 border border-green-500/40 hover:bg-green-500/30 text-sm font-medium" data-action="approve" data-id="${t.id}">✓ Approve</button>
          `}
          <button class="td-action px-3 py-1.5 rounded-lg text-red-400 hover:text-red-300 hover:bg-red-500/10 text-sm" data-action="dismiss" data-id="${t.id}">Dismiss</button>
        </div>
        <p class="text-[11px] text-slate-400 leading-relaxed">
          ${hasDraft ? 'Per PRD §7.1: one-click <strong class="text-slate-200">Approve &amp; send</strong> fires the draft immediately and auto-closes the task.' : 'Approving keeps the task in the active list. Dismissing removes it from default queues but the audit trail is preserved.'}
        </p>
      </div>
    ` : '';
    ...
    if (t.task_type === 'messaging' && t.draft) {
      typeBlock = `
        <div class="bg-slate-950 border border-slate-800 rounded-lg p-3">
          <div class="text-[10px] uppercase tracking-wider text-slate-500 mb-1">Drafted reply</div>
          <p class="text-sm text-slate-200 leading-relaxed font-mono whitespace-pre-line">${t.draft}</p>
        </div>
      `;
      ...
      typeBlock = `
        <div class="grid grid-cols-2 gap-2">
          <div class="bg-slate-950 border border-slate-800 rounded-lg p-3">
            <div class="text-[10px] uppercase tracking-wider text-slate-500">Action</div>
            <div class="text-sm text-slate-200 mt-0.5">${t.subtype}</div>
          </div>
          <div class="bg-slate-950 border border-slate-800 rounded-lg p-3">
            <div class="text-[10px] uppercase tracking-wider text-slate-500">Vendor / supplier</div>
            <div class="text-sm text-slate-200 mt-0.5">${(t.evidence[0] && t.evidence[0].label) || 'TBD'}</div>
          </div>
        </div>
      `;
    } else if (t.task_type === 'disruption-response') {
      typeBlock = `
        <div class="bg-red-500/5 border border-red-500/30 rounded-lg p-3">
          <div class="text-[10px] uppercase tracking-wider text-red-400 mb-1">Trigger event</div>
          <div class="text-sm text-slate-100">${t.subtype}</div>
        </div>
      `;
    } else if (t.task_type === 'trip-prep') {
      typeBlock = `
        <div class="bg-slate-950 border border-slate-800 rounded-lg p-3">
          <div class="text-[10px] uppercase tracking-wider text-slate-500 mb-1">Trip-prep step</div>
          <div class="text-sm text-slate-200">${t.subtype}</div>
        </div>
      `;
    }
    ...
    const activityHtml = (t.activity || []).map(a => `
      <div class="flex gap-3 py-2.5 border-b border-slate-800 last:border-b-0">
        <div class="w-1.5 h-1.5 rounded-full bg-slate-600 mt-1.5 flex-shrink-0"></div>
        <div class="flex-1 min-w-0">
          <div class="flex items-baseline justify-between gap-2">
            <div class="text-xs text-slate-200">${a.actor} · <span class="text-slate-400">${a.event}</span></div>
            <div class="text-[10px] text-slate-500 font-mono flex-shrink-0">${a.ts}</div>
          </div>
          ${a.detail ? `<div class="text-[11px] text-slate-500 mt-0.5">${linkifyTaskIds(a.detail)}</div>` : ''}
        </div>
      </div>
    `).join('');
    ...
    const evidenceHtml = (t.evidence || []).map(e => `
      <a href="#evidence" class="evidence-chip inline-flex items-center gap-1.5 px-2 py-1 rounded bg-slate-800 border border-slate-700 text-[11px] text-slate-300 hover:text-orange-300 hover:border-orange-500/40 transition-colors mr-2 mb-1" data-label="${e.label}">
    ...
    document.getElementById('shared-drawer-content').innerHTML = `
      <div class="sticky top-0 bg-slate-900 border-b border-slate-800 px-5 py-4 flex items-center justify-between z-10">
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] bg-slate-800 border border-slate-700 ${src.color}">${src.svg}<span class="text-slate-300">${src.label}</span></span>
          <span class="text-[11px] text-slate-500">${TASK_TYPE_LABELS[t.task_type]} · ${t.subtype}</span>
        </div>
        <div class="flex items-center gap-1">
          <a href="task-board.html#task=${t.id}" class="p-1 rounded hover:bg-slate-800 text-slate-500 hover:text-slate-200" title="Open in Tasks board (full page)">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
          </a>
          <button id="shared-drawer-close" class="p-1 rounded hover:bg-slate-800 text-slate-400 hover:text-slate-200">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
        </div>
      </div>
      <div class="p-5 space-y-5">
        <div>
          <h3 class="text-lg font-semibold text-slate-100 leading-snug">${t.title}</h3>
          <div class="flex items-center gap-3 mt-2 text-xs text-slate-400 flex-wrap">
            ...
            <span>Due: <span class="text-slate-200">${t.due_at}</span></span>
            <span class="text-slate-700">·</span>
            <span>Priority: <span class="text-slate-200">${PRIORITY_STYLES[t.priority].label}</span></span>
            <span class="text-slate-700">·</span>
            <a href="task-board.html#task=${t.id}" class="task-id-link font-mono text-slate-600 hover:text-orange-400">${t.id}</a>
          </div>
        </div>
        ...
        <div class="bg-slate-800/40 border border-slate-800 rounded-lg p-4">
          <div class="flex items-start gap-2 mb-2">
            <svg class="w-4 h-4 text-orange-400 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            <div class="flex-1">
              <div class="text-[10px] uppercase tracking-wider text-slate-500 mb-1">Rationale</div>
              <p class="text-sm text-slate-200 leading-relaxed">${linkifyTaskIds(t.rationale)}</p>
            </div>
          </div>
          ${evidenceHtml ? `<div class="mt-3 pt-3 border-t border-slate-800"><div class="text-[10px] uppercase tracking-wider text-slate-500 mb-2">Evidence</div><div>${evidenceHtml}</div></div>` : ''}
        </div>
        ...
        ${t.ai_reasoning ? `
          <div class="bg-slate-950 border border-slate-800 rounded-lg p-4">
            <div class="flex items-start gap-2">
              ...
              <div class="flex-1">
                <div class="text-[10px] uppercase tracking-wider text-slate-500 mb-1">AI reasoning</div>
                <p class="text-xs text-slate-400 leading-relaxed">${linkifyTaskIds(t.ai_reasoning)}</p>
                ${t.confidence ? `<div class="text-[10px] text-slate-500 mt-2">Confidence: <span class="font-mono text-amber-400">${t.confidence}%</span></div>` : ''}
              </div>
            </div>
          </div>
        ` : ''}
        ...
        <div>
          <div class="text-[10px] uppercase tracking-wider text-slate-500 mb-2">Activity</div>
          <div class="bg-slate-950 border border-slate-800 rounded-lg px-4">
            ${activityHtml}
          </div>
        </div>
      </div>
      ...
      ${isPending ? '' : `
      <div class="sticky bottom-0 bg-slate-900 border-t border-slate-800 p-4 flex items-center gap-2 flex-wrap">
        ${hasDraft && t.status !== 'completed' ? `
          <button class="td-action px-3 py-1.5 rounded-lg bg-orange-500 text-white hover:bg-orange-600 text-sm font-medium" data-action="send-draft" data-id="${t.id}">Send draft now</button>
          <button class="td-action px-3 py-1.5 rounded-lg bg-slate-800 text-slate-200 hover:bg-slate-700 text-sm" data-action="open-composer" data-id="${t.id}">Edit in composer</button>
        ` : `
          <button class="td-action px-3 py-1.5 rounded-lg bg-orange-500 text-white hover:bg-orange-600 text-sm font-medium" data-action="complete" data-id="${t.id}">Complete</button>
        `}
        <button class="td-action px-3 py-1.5 rounded-lg bg-slate-800 text-slate-200 hover:bg-slate-700 text-sm" data-action="snooze" data-id="${t.id}">Snooze</button>
  ...
  const EVIDENCE_TEMPLATES = {
    message: (label) => `
      <div class="text-[11px] text-slate-500 mb-3">Message thread excerpt · ${label}</div>
      <div class="space-y-3">
        <div class="flex items-start gap-2.5">
          <div class="w-8 h-8 rounded-full bg-orange-500/20 text-orange-300 flex items-center justify-center text-[10px] font-bold flex-shrink-0">CL</div>
          <div class="flex-1 min-w-0">
            <div class="flex items-baseline gap-2 mb-1">
              <span class="text-xs font-medium text-slate-200">Client</span>
              <span class="text-[10px] text-slate-500">${label.includes('·') ? label.split('·')[1].trim() : '2:14 PM'}</span>
            </div>
            <div class="bg-slate-800 rounded-2xl rounded-tl-sm px-3.5 py-2.5 text-sm text-slate-200 leading-relaxed">
    ...
    tool: (label) => {
      if (label.toLowerCase().includes('seats.aero')) {
        return `
          <div class="text-[11px] text-slate-500 mb-3">Award search · ${label}</div>
          <div class="bg-slate-950 border border-slate-800 rounded-lg overflow-hidden">
            <div class="px-3 py-2 border-b border-slate-800 bg-slate-800/30 text-[11px] text-slate-400 flex items-center justify-between">
              <span>IAD → HND · 2026-04-30 · 2 pax · First</span>
              <span class="font-mono text-green-400">2 award seats found</span>
            </div>
            <table class="w-full text-xs">
              <thead class="bg-slate-800/20 text-slate-500 text-[10px] uppercase tracking-wider">
              ...
              <tbody class="divide-y divide-slate-800">
                <tr><td class="px-3 py-2 font-mono text-slate-200">JL061</td><td class="px-3 py-2 text-slate-300">IAD-HND</td><td class="px-3 py-2 text-slate-400">11:55a → 3:35p+1</td><td class="px-3 py-2"><span class="px-1.5 py-0.5 rounded text-[10px] bg-orange-500/15 text-orange-300 border border-orange-500/30">First</span></td><td class="px-3 py-2 font-mono text-right text-amber-400">82,500</td></tr>
                <tr><td class="px-3 py-2 font-mono text-slate-200">JL061</td><td class="px-3 py-2 text-slate-300">IAD-HND</td><td class="px-3 py-2 text-slate-400">11:55a → 3:35p+1</td><td class="px-3 py-2"><span class="px-1.5 py-0.5 rounded text-[10px] bg-amber-500/15 text-amber-300 border border-amber-500/30">Business</span></td><td class="px-3 py-2 font-mono text-right text-amber-400">65,000</td></tr>
              </tbody>
            </table>
          </div>
        `;
      ...
      if (label.toLowerCase().includes('ita') || label.toLowerCase().includes('alt')) {
        return `
          <div class="text-[11px] text-slate-500 mb-3">Alternative routings · ITA Matrix</div>
          <div class="bg-slate-950 border border-slate-800 rounded-lg p-3 space-y-2">
            <div class="flex items-center justify-between p-2 rounded bg-slate-800/30"><span class="text-sm text-slate-200"><span class="font-mono">LH444</span> · IAD → MUC → FRA</span><span class="text-[11px] font-mono text-slate-400">+1d · Y avail</span></div>
            <div class="flex items-center justify-between p-2 rounded bg-slate-800/30"><span class="text-sm text-slate-200"><span class="font-mono">UA960 + LH107</span> · IAD → MUC → FRA</span><span class="text-[11px] font-mono text-slate-400">+8h · Y avail</span></div>
            <div class="flex items-center justify-between p-2 rounded bg-slate-800/30"><span class="text-sm text-slate-200"><span class="font-mono">LX65 + LX1066</span> · IAD → ZRH → FRA</span><span class="text-[11px] font-mono text-slate-400">+14h · J avail</span></div>
          </div>
        `;
      }
      return `<div class="text-[11px] text-slate-500 mb-3">External tool result · ${label}</div><div class="bg-slate-950 border border-slate-800 rounded-lg p-4 text-sm text-slate-300">Generic tool result panel. In production this opens the corresponding tool view.</div>`;
    },
    email: (label) => `
      <div class="text-[11px] text-slate-500 mb-3">Vendor / system notification · ${label}</div>
      <div class="bg-slate-950 border border-slate-800 rounded-lg overflow-hidden">
        <div class="px-4 py-3 border-b border-slate-800 bg-slate-800/30">
          <div class="text-[10px] uppercase tracking-wider text-slate-500 mb-0.5">From</div>
          <div class="text-sm text-slate-200">${getEmailSender(label)}</div>
          <div class="text-[10px] uppercase tracking-wider text-slate-500 mt-2 mb-0.5">Subject</div>
          <div class="text-sm text-slate-200">${getEmailSubject(label)}</div>
        </div>
        <div class="p-4 text-sm text-slate-300 leading-relaxed">${getEmailBody(label)}</div>
      </div>
    `,
    page: (label) => `
      <div class="text-[11px] text-slate-500 mb-3">Internal page · ${label}</div>
      <div class="bg-slate-950 border border-slate-800 rounded-lg p-4">
        <div class="flex items-start justify-between mb-3 pb-3 border-b border-slate-800">
          <div>
            <div class="text-base font-semibold text-slate-100">${label}</div>
            <div class="text-[11px] text-slate-500 mt-0.5">${getPageMeta(label)}</div>
          </div>
          <span class="text-[10px] font-mono text-slate-600">cc://${label.toLowerCase().replace(/[^a-z0-9]+/g,'-')}</span>
        </div>
        <div class="grid grid-cols-2 gap-3 text-xs">
          ${getPageFields(label).map(f => `<div><div class="text-[10px] uppercase tracking-wider text-slate-500">${f[0]}</div><div class="text-slate-300 mt-0.5">${f[1]}</div></div>`).join('')}
        </div>
      </div>
    `,
    dashboard: (label) => `
      <div class="text-[11px] text-slate-500 mb-3">Dashboard · ${label}</div>
      <div class="bg-slate-950 border border-slate-800 rounded-lg p-4">${getDashboardContent(label)}</div>
    `,
  };
  ...
  function getDashboardContent(label) {
    ...
    if (l.includes('sla')) return `
      <div class="text-[10px] uppercase tracking-wider text-slate-500 mb-3">SLA timers · VVIP threshold = 2h</div>
      <div class="space-y-2">
        <div class="flex items-center gap-3"><span class="text-xs text-slate-300 flex-1">Greer family · WhatsApp</span><div class="flex-1 bg-slate-800 rounded-full h-2 overflow-hidden"><div class="h-full bg-red-500" style="width: 82%"></div></div><span class="text-[11px] font-mono text-red-400">1h 38m</span></div>
        <div class="flex items-center gap-3"><span class="text-xs text-slate-300 flex-1">Whitman · Stream Chat</span><div class="flex-1 bg-slate-800 rounded-full h-2 overflow-hidden"><div class="h-full bg-amber-500" style="width: 45%"></div></div><span class="text-[11px] font-mono text-amber-400">54m</span></div>
      </div>
    `;
    return `<div class="text-sm text-slate-300">Dashboard view. Live chart renders here in production.</div>`;
  }
  ...
  function showSharedToast(msg, undoFn) {
    ...
    toast.className = 'shared-toast bg-slate-800 border border-slate-700 rounded-lg shadow-xl px-4 py-2.5 min-w-[280px] max-w-[420px] flex items-center gap-3';
    toast.innerHTML = `
      <div class="flex-1 text-sm text-slate-200">${msg}</div>
